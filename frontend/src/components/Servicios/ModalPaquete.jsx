...
export default function ModalPaquete({ show, onClose, negocio, paquete, onSaved }) {
  ...
  async function uploadIfNeeded() {
    ...
    const upRes = await fetch("https://oral-susan-utt-eab6c28f.koyeb.app/api/uploads", {
      method: "POST",
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (paquete?.id) {
        // EDITAR
        const res = await fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/packages/${paquete.id}`, {
          method: "PUT",
          headers: headersJson,
          body: JSON.stringify({
            name: name.trim(),
        ...
        // CREAR
        const res = await fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/${negocio.id}/packages`, {
          method: "POST",
          headers: headersJson,
          body: JSON.stringify({
            name: name.trim(),
          ...
          <Field>
            ...
            <Preview>
              {file
                ...
                : photoPath
                  ? <img src={`https://oral-susan-utt-eab6c28f.koyeb.app/${photoPath}`} alt="preview"/>
                  : "Sin imagen"}
            </Preview>
          </Field>
