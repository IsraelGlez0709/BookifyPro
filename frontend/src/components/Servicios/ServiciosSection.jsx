...
export default function ServiciosSection({ negocio }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/${negocio.id}`, {
    ...
    try {
      ...
      if (payload.id) {
        ...
        const res = await fetch(
          `https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/services/${payload.id}`,
          {
            method: "PUT",
            headers,
            body: JSON.stringify({
        ...
        // CREAR
        const res = await fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/services`, {
          method: "POST",
          headers,
          body: JSON.stringify({
  ...
  async function handleDeletePaquete(pkg) {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/businesses/packages/${pkg.id}`, {
        method: "DELETE",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) throw new Error(await res.text());
