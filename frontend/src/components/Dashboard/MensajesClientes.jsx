    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(
          `https://oral-susan-utt-eab6c28f.koyeb.app/api/messages/dashboard-previews?business_id=${negocio.id}`,
        ...
        {rows.map((m) => {
          ...
          return (
            <Msg key={m.conversation_id} onClick={go} title={`Ver chat con ${m.customer_name}`}>
              <Avatar>
                {m.customer_photo ? (
                  <img src={`https://oral-susan-utt-eab6c28f.koyeb.app/${m.customer_photo}`} alt={m.customer_name} />
                ) : initials}
              </Avatar>
              <Texts>
                <NameRow>
