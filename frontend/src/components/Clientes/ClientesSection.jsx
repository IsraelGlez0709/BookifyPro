...
export default function ClientesSection({ negocio }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://oral-susan-utt-eab6c28f.koyeb.app/api/clients?business_id=${negocio.id}`, {
  ...
  const handleGuardarCliente = async (nuevoCliente) => {
    ...
    const token = localStorage.getItem("token");
    const res = await fetch("https://oral-susan-utt-eab6c28f.koyeb.app/api/clients", {
      method: "POST",
      headers: {
